...
```html
<audio scr="오디오 경로" controls autoplay loop></audio>
```
- controls : 재생도구 출력 여부
- autoplay : 자동 재생여부 지정
- loop : 반복 재생여부 지정
...
3. 하위 디렉터리에 있는 경우 : 해당하는 디렉터리명/파일명.확장자
...
1. colspan : 열 병합(가로 병합)
2. rowspan : 행 병합(세로 병합)

## 접근성 향상을 위한 요소

1. th(Table Header) : 제목 셀 지정(가운데 지정 및 굵은 글씨 됨)
2. caption : 테이블의 제목
3. thead(제목) / tbody(내용) / tfoot(결과): 각 셀들의 역할 별 구분
...
## GET, POST
  ...
  - GET
   - 데이터를 불러올 때 사용.
   - 데이터의 안정성이 낮음.
   - 데이터를 URL에 노출시켜서 서버에 전달.
   - 데이터 전달 속도가 빠르다.

 - POST
  - 데이터를 보낼 때 사용.
  - 데이터의 안전성이 높음.
  - 데이터를 URL에 노출시키지 않고 서버에 전달.(EX. 로그인시 아이디 비밀번호 노출되지X)
  - GET보다 데이터 전달 속도가 느림.

## input

 - 사용자로부터 데이터를 입력 받기 위한 태그.
 - 속성 type의 값에 따라서 input이 변형.
 - 속성 값 종류

1. **text** : 한 줄의 텍스트를 입력 받을 수 있음.
 
 - size : 입력 칸의 길이(보이는 글자 수)
 - maxlength : 최대 입력 가능한 글자 수
 - minlenght : 최소 입력 가능한 글자 수
 - value : 입력 값

2. **password** : 비밀번호를 입력 받을 수 있음.
...
3. **checkbox** : 여러 항목 중 여러 가지를 선택 할 수 있음.

 - value : check된 상태로 전송을 하면 value의 값이 전송.
 - checked : checkbox가 체크된 상태로 페이지가 열림.

4. **radio** : 여러 항목 중 한 가지만 선택 가능.

 - name : name이 같은 애들끼리 중에서 하나만 체크.

5. **file** : 파일을 첨부 가능.

 - multilple 속성을 통해 파일을 다중으로 선택 가능.
 - accept : 파일을 선택할 때 보여지는 파일들의 종류 지정.

6. **hidden** : 정보를 가지고 있어야하지만, 사용자에게 안보이게 할 때 사용.

7. **button** : 기본적으로 가진 기능이 없어 다른 기능과 연결해서 사용.

 - value : 버튼의 text를 바꿀 수 있음.

8. **submit** : 데이터를 전송할 때 사용하는 버튼.

 - form 태그의 method 속성에 지정된 전송 방식으로 action속송에 지정한 서버로 입력한 값들을 전송.

9. **reset** : form에서 입력한 값을 모두 초기화.

## 추가된 Type

1. 날짜 관련 Type

- **datetime** or **datetime-local** : 년/월/일/시/분/초에 대한 데이터로 입력 받을 수 있음.
- **week** : 주에 대한 데이터를 입력 받을 수 있음.
- **month** : 년/월에 대한 데이터를 입력 받을 수 있음.
- **date** : 년/월/일에 대한 데이터를 입력 받을 수 있음.
- **time** : 시간(시/분/초)에 대한 데이터를 입력 받을 수 있음.

2. **email** : 이메일 주소를 입력 받을 수 있음.

- 입력받은 값이 email 주소 형식이 맞는지 자동으로 체크.

3. **url** : URL을 입력 받을 수 있음.

4. **search** : 검색 값을 입력 받을 수 있음.

- text 타입과 다르게 검색어 입력시 오른쪽 끝에 'X'가 표시가 되고 누르면 검색어를 쉽게 지울수 있다.

5. **color** : 색상을 선택 가능.

6. **number** : 숫자를 입력받을 수 있음.

- min : 입력할 수 있는 최소값(기본값 : 0)
- max : 입력할 수 있는 최대값(기본값 : 100)
- step : 증감되는 숫자의 간경을 저장(기본값 : 1)
- value : 표시할 초기값

7. **range** : 슬라이드바를 통해 숫자를 입력 받을 수 있음.

8. **tel** : 전화번호를 입력 받을 수 있음.

- 모바일 기기의 경우 가상 키보드 배열이 전화번호를 입력받을 수 있도록 바뀜.
...
- 대표적인 인라인 태그는 sapn, img, input, button 태그 등이 있다.
- 인라인 태그 안에는 인라인 태그만 사용할 수 있다.

## div(100%)와 sapn(글씨영역만큼)
