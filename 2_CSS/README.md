...
- 6자리는 앞에서부터 두자리씩 묶어서 Red, Green, Blue 로 표시
...
```css
rgb(red(0~255), green(0~225), blue(0~225));
rgba(red(0~255), green(0~225), blue(0~225), alpha(0~1));
```
...
- **no-repeat** : 배경 이미지를 한 번만 출력하고 반복하지 않습니다.
...
- 수직 위치 : left, center, right, 크기값(px), 백분율로 표시

## background-origin

- 배경 이미지를 배치할때 기준을 지정하는 속성
...
- 웹페이지가 위아래로 움직여도 배경이미지는 움직이지 않고 고정하는 속성
...
- collapse : 표의 행, 열, 행 그룹 등에서 지정하면 서로 겹치도록 지정한다. 그외의 영역에서 사용하면 hidden처럼 처리된다.

## z-index

- 요소 위에 요소를 쌓을 때 쌓는 순서를 지정하는 속성
- 요소에 z-index 값이 클수록 위에 쌓인다.
- 무조건 맨위에 표시해야하는 경우 큰 값으로 지정하기도 한다.
...
- cloumn : 위에서 아래로 순차적으로 배치
- cloumn-reverse : 아래에서 위로 순차적으로 배치


## flex-wrap

- 크기 조절에 따라 아이템 줄바꿈을 어떻게 할지 속성
...
- flex-direction과 flex-wrap을 한꺼번에 저장할 수 있는 단축 속성
...
```css
.container {
    jusify-content: flex-start or flex-end or center or space-between or space-around or space-evenly;
}
```
...
- sapce-around : 각 아이템의 좌우에 균일한 간격 생성
- space-evenly : 아이템들의 사이와 양 끝에 균일한 간격 생성(안먹히는 브라우저도 있음.)

## align-items

- 수직축 방향으로 정렬
(direction에 따라서 방향이 변경되어서 사용)
...
```css
.container {
    align-content: stretch or flex-start or flex-end or center or space-between of space-around or space-evenly;
}
```

## flex-basic

-flex 아이템의 기본 크기 설정 (flex-direction이 row일때는 너비, column일때는 높이)
...
- flex-grow, flex-shrink, flex-basis 를 한번에 사용할 수 있는 단축 속성
...
```css
.item {
    align-self: flex-start or flex-end or center or baseline(높이가 다르면 확인가능) or stretch;
}
```
...
.container {
    ...
    grid-template-columns: 1fr 1fr 1fr; // fr: 구역
    grid-template-columns: repeat(3, 1fr); // 위와 같은 의미
...
@keyframes 애니메이션이름{
    ...
    선택자 {
        animation: name duration iteration-count timing-function dlay direction;
    }
```
